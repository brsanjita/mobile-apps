...
class UserController {
  ...
  static Future<bool> signUpWithEmail(User user, String password) async {
    ...
    http.Response response = await trustedHttp.post(
  ...
  static Future<bool> requestOTP(MobileNumber mobileNumber) async {
    ...
    mutation{
      requestLoginOTP(mobileNo:"${mobileNumber
        .number}",countryCode:"${mobileNumber.countryCode}")
    } ''';
    TrustedHttp trustedHttp = TrustedHttp();
    http.Response response = await trustedHttp.post(
       RemoteConfig.BASE_URL, body: jsonEncode({"query": query}));
    ...
    var json = jsonDecode(response.body);
    if (json["data"] != null && json["data"]["requestLoginOTP"] != null &&
        json["data"]["requestLoginOTP"]) {
      return true;
    }
    return false;
  }
  ...
  static Future<bool> verifyOTP(MobileNumber mobileNumber, String otp) async {
    String query = '''
    mutation{
      verifyLoginOTP(mobileNo:"${mobileNumber
        .number}",countryCode:"${mobileNumber.countryCode}",otp:"$otp"){
      token
      }
    } ''';
    TrustedHttp trustedHttp = TrustedHttp();
